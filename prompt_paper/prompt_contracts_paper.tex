...
\textbf{Artifacts:} \texttt{input\_final.txt} (prompt with constraints), \texttt{output\_raw.txt} (model response), \texttt{output\_norm.txt} (post-repair), \texttt{run.json} (metadata: timestamp, seed, checks, repair ledger, SHA-256 prompt hash).
...
\textbf{Tasks:} Classification (English/German), Extraction (finance), RAG Q\&A (wiki), Summarization (news), Tool-calls (API). Total: 520 labeled fixtures (100+ per task). \textbf{Domains:} Business, finance, news, Wikipedia, tool use. \textbf{Languages:} English (320), German (200). \textbf{Models:} GPT-4o-mini (enforce), Mistral-7B (assist), GPT-4o (judge). \textbf{Metrics.} (1) \textit{validation\_success}: Percentage passing all checks with Wilson 95\% CI, (2) \textit{task\_accuracy}: Exact match to gold labels, (3) \textit{repair\_rate}: Fraction requiring normalization, (4) \textit{semantic\_change\_rate}: Fraction where repairs altered semantics, (5) \textit{latency\_ms}: Mean generation time, (6) \textit{overhead\_pct}: Check execution time percentage. \textbf{Reproducibility:} Fixed seed=42, temp=0, pinned dependencies (Python 3.11.7, torch 2.0.1, sentence-transformers 2.2.2). Docker: \texttt{prompt-contracts:0.3.2} with PYTHONHASHSEED=42, OMP\_NUM\_THREADS=1. Reproduction: \texttt{docker run prompt-contracts:0.3.2 make eval-full}.
...
\textbf{Transformations.} PCSL applies ordered normalizations: (1)~\texttt{strip\_markdown\_fences}: Remove \texttt{```json```} wrappers, (2)~\texttt{strip\_whitespace}: Trim whitespace, (3)~\texttt{normalize\_newlines}: Unify line endings.
...
\begin{table}[H]
\centering
\caption{System Comparison (n=50 fixtures, McNemar \& bootstrap tests)}
\label{tab:fair_comparison}
\scriptsize
\begin{tabular}{@{}lccccc@{}}
\toprule
\textbf{System} & \textbf{Val. Succ.} & \textbf{Setup Time} & \textbf{Latency (ms)} & \textbf{Sig. (vs PCSL)} \\
\midrule
PCSL & 92\% [89\%, 95\%] & 5 min (2 iter) & 142 $\pm$ 18 & - \\
CheckList & 87\% [83\%, 91\%] & 18 min (6 iter) & 168 $\pm$ 24 & p=0.08 (n.s.)$^a$ \\
Guidance & 89\% [85\%, 93\%] & 12 min (4 iter) & 155 $\pm$ 21 & p=0.21 (n.s.)$^a$ \\
\bottomrule
\end{tabular}
\vspace{1mm}
\scriptsize $^a$McNemar test (paired binary). Latency: bootstrap $\Delta$CI for PCSL vs CheckList: [-28ms, -15ms] (significant).
\end{table}

\textbf{Findings:} PCSL achieves comparable validation success (no significant difference via McNemar, $\alpha$=0.05) with 3.6$\times$ faster setup and significantly lower latency (bootstrap difference CI excludes 0). The formal specification language (PCSL JSON) reduces integration time compared to manual test writing (CheckList) or constraint programming (Guidance).
