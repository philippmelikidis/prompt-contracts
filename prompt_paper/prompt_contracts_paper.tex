...
% Standardize all tables to paragraph width
\newcommand{\standardtable}[2]{
  \begin{table}[H]
  \centering
  \caption{#1}
  \tiny
...
\midrule
PD & Test Item (§7.1) & - \\
ES & Test Conditions (§7.2) & Art. 15 \\
EP & Test Case (§7.3) & Art. 9 \\
\breakablefunction{save\_io} & Test Log (§8.3) & Art. 12 \\
Negotiation & Test Env (§8.1) & Art. 13 \\
N-sampling+CI & Statistical (29119-4) & Art. 15 \\
Repair ledger & Incident (§8.4) & Art. 14 \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
Val. Success & 96.2\% & Wilson & [0.941, 0.978] & Full eval & v0.4.0 \\
Val. Success (static) & 91.5\% & Bootstrap & [0.888, 0.936] & Offline sim & v0.3.1 \\
...
Sem. Change Rate & 1.4\% & Wilson & [0.8\%, 2.3\%] & Human audit & v0.4.0 \\
Repair Rate & 28.5\% & Wilson & [0.245, 0.328] & Live eval & v0.4.0 \\
\bottomrule
\end{tabular}
\vspace{1mm}
\tiny H–H = Human-Human agreement, J–H = Judge-Human agreement. CI = Confidence Interval.
\end{table}
...
\end{tabular}
\vspace{1mm}
\tiny\(^*\)OH\% = Overhead\% = (check execution time / total latency) \(\times\) 100
\end{table}
...
\end{tabular}
\vspace{1mm}
\tiny\(^*\)CI Width = upper bound - lower bound (95\% bootstrap percentile)
\end{table}
...
\end{tabular}
\vspace{1mm}
\tiny\(^*\)Std = empirical standard deviation across 5 seeds
\end{table}
...
\end{tabular}
\vspace{1mm}
\tiny Drift $\Delta$ = week 4 mean - week 1 mean. CI Width = 95\% Wilson interval width.
\end{table}
