...
\textbf{Tasks:} Classification (EN/DE), Extraction (finance), RAG Q\&A (wiki), Summarization (news), Tool-calls (API). Total: 520 fixtures (100+ per task). \textbf{Domains:} Business, finance, news, Wikipedia. \textbf{Languages:} English (320), German (200). \textbf{Models:} GPT-4o-mini (enforce), Mistral-7B (assist), GPT-4o (judge).

\textbf{Metrics:} (1)~\textit{validation\_success}: Passing all checks with Wilson 95\% CI, (2)~\textit{task\_accuracy}: Exact match, (3)~\textit{repair\_rate}: Normalization fraction, (4)~\textit{semantic\_change\_rate}: Semantic alteration fraction, (5)~\textit{latency\_ms}: Generation time, (6)~\textit{overhead\_pct}: Check time.

\textbf{Reproducibility:} seed=42, temp=0, Python~3.11.7, torch~2.0.1, sentence-transformers~2.2.2. Docker \texttt{prompt-contracts:0.3.2} with \texttt{PYTHONHASHSEED=42}. Run: \texttt{docker run prompt-contracts:0.3.2 make eval-full}.
