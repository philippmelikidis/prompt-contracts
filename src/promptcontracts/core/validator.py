    ...
    for check_type in check_order:
        ...
        for check in matching_checks:
            if check_type == 'pc.check.json_valid':
                ...
                casing_note = " (lowercase)" if not case_insensitive and all(v.islower() for v in allowed if isinstance(v, str)) else ""
                constraints.append(f"- `{field_name}` MUST be exactly one of: {', '.join(map(str, allowed))}{casing_note}.")
