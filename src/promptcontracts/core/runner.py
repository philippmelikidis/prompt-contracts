...
from .validator import Validator, CheckRegistry, normalize_output, derive_json_schema_from_es, build_constraints_block
...
class ContractRunner:
    ...
    def __init__(self, pd: Dict[str, Any], es: Dict[str, Any], ep: Dict[str, Any], save_io_dir: Optional[str] = None):
        """
        ...
        self.auto_repair_cfg = execution.get('auto_repair', {
    ...
    def _determine_effective_mode(self, requested_mode: str, adapter_capabilities) -> tuple[str, bool]:
        """
    ...
    def _save_artifacts(self, target_id: str, fixture_id: str, final_prompt: str, 
                       raw_output: str, normalized_output: str, metadata: Dict[str, Any]):
    ...
    def _validate_response(self, response_text: str, parsed_json: Any = None) -> List[Dict[str, Any]]:
    ...
    def _run_fixture_with_retry(self, adapter, schema: Optional[Dict], final_prompt: str, 
        ...
        for attempt in range(self.max_retries + 1):
            ...
            if attempt < self.max_retries:
                ...
                # Apply auto-repair
                normalized_output, repair_details = normalize_output(raw_output, self.auto_repair_cfg)
    ...
    def run(self) -> Dict[str, Any]:
        ...
        for target in targets:
            ...
            effective_mode, is_nonenforceable = self._determine_effective_mode(self.exec_mode, capabilities)
            ...
            for fixture in fixtures:
                ...
                if self.save_io_dir:
                    ...
                    artifact_path = self._save_artifacts(
                        target_id, fixture_id, final_prompt,
                        fixture_result['raw_output'], fixture_result['normalized_output'],
                        metadata
                    )
